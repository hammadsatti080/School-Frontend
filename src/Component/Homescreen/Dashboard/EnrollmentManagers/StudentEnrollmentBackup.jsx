...
const StudentEnrollmentBackup = () => {
  ...
  useEffect(() => {
    if (selectedClass) {
      setFilteredEnrollments(
        enrollments.filter(
          (item) => Number(item.studentClass) === Number(selectedClass)
        )
      );
    } else {
  ...
  const handleEdit = (item) => {
    setEditing(item._id);
    setFormData({ ...item }); // clone the item into formData
  };

  const handleUpdate = async () => {
    if (!formData.gender || !formData.contactNo) {
      alert("Please fill required fields before updating!");
      return;
    }

    await fetch(`http://localhost:5000/api/enrollments/${editing}`, {
      method: "PUT",
      headers: {
  ...
  return (
    <div className="container mt-4">
      <h4 className="mb-3">Enrollment List</h4>
